...
if (import.meta.main) {
  ...
  if (!input) {
    input = prompt("Please enter a day (0 - 25) to fetch the puzzle outcome", '') || '';
  }
  if (!input) {
  ...
  console.log()
  console.log(`Puzzles took: %c${(timeEnd - timeStart)} milliseconds %cin total`, 'color: cyan', 'color: normal')
